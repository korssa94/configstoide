...
class Tdipar:
    ...
    def get_gvl_string(self):
        res = ""
        # Дополнительная инициализация для приема через AI
        if self.circuit_control in ["1", "2"]:
            res += f"\t{{attribute 'symbol' := 'none'}}\n"
            res += f"\t{self.alg_name}_ai : Tai_to_di := (par_type := {self.circuit_control});\n"
        
        marker = self._generate_marker()
        inv_str = " := (inv := TRUE)" if self.signal_type == "НЗ СК" else ""
        class_type = "Tdipar_safety" if self.paz else "Tdipar"
        subsys_comment = self.subsystem if self.subsystem else ""
        
        res += f"\t{marker}\n"
        # ИСПРАВЛЕНИЕ: Убраны лишние пробелы в комментариях
        res += f"\t{self.alg_name} : {class_type}{inv_str}; //{self.description}/{self.tech_name}/{subsys_comment}"
    ...
    def get_st_string(self):
        ...
        if self.device and self.crate and self.module and self.channel:
            try:
                ...
                ch_idx = int(self.channel) - 1 # Для обращений через индекс массива (.VALUE.0)
            except ValueError:
